...
\begin{equation}
	\adjustbox{width=.8\columnwidth}{\ensuremath{\inputtikz{sd-csd-example}}}
	\label{eq:CSD}
	\tag{CSD}
\end{equation}
...
However, we could solve this conundrum by adding to our set of equations the following rule, which we denote as the \emph{Currying} rule:
\begin{equation}
	\inputtikz{sd-curry}
	\label{eq:curry}\tag{Currying}
\end{equation}
...
Take the equation presented in Figure \ref{fig:sd-kleisli-composition}.
\begin{figure*}
	\begin{equation*}
		\inputtikz{sd-kleisli-composition}
	\end{equation*}
	\caption{Example of easy composition of kleisli arrows in the string diagram format, with no derivation concerns.}
	\label{fig:sd-kleisli-composition}
\end{figure*}
...
\begin{description}
	\item[The \emph{Elevator} Equations] are a consequence of Theorem \ref{thm:isotopy} but also highlight one of the most important properties of string diagrams in their modelisation of multi-threaded computations: what happens on one string does not influence what happens on another in the same time:
	      \begin{equation}
		      \columneq{\inputtikz{sd-elevator}}
		      \label{eq:elevator}
	...
	\item[The \emph{Snake} Equations] are a rewriting of the categorical diagrams equations \eqref{eq:zigzag} and \eqref{eq:zagzig} of Paragraph \ref{par:adjunctions} which are the defining properties of an adjunction.
	      ...
	      \begin{equation}
		      \columneq{\inputtikz{sd-snake1}}
		      \tag{\rotatebox[origin=c]{90}{\emoji{snake}}}
		      \label{eq:snek1}
	      \end{equation}
	      \vspace{-12pt}
	      \begin{equation}
		      \columneq{\inputtikz{sd-snake2}}
	      ...
	      \begin{category}[]
		      \eqref{eq:zigzag}\ar[d, "\texttt{\textbackslash reflectbox}"']\ar[r, "\texttt{sd}"]  & \eqref{eq:snek1}\ar[d, "\texttt{\textbackslash reflectbox}"]  \\
		      \eqref{eq:zagzig}\ar[r, "\texttt{sd}"'] & \eqref{eq:snek2}
	      \end{category}
