...
way of using string diagrams for syntactic reductions, we can use our effects
to improve the typing by asking that effects arising from a computation are
sent to an axis orthogonal to the one used to represent the sentence, allowing
for a reduction on a second axis in a second time.
Since we provide a compositional addition to a denotation system, it amounts to
replacing/improving the category of meaning in the product category with the
pregroup category and thus means we can integrate our theory inside their
formalism.
Indeed, we start from a meaning category $\mC$ and then build on it, instead
of providing a full on denotation system\footnote{Section \ref{sec:language}
	proposes a translation of the english language based on usual denotations, but
	our system could be applied onto a fully different representation of meaning,
	as long as it can be mapped to a cartesian closed category.}.
Moreover, when seeing their parsing trees as string diagrams and using the fact
that we can always assume natural transformations (or $2$-cells) to be at
different heights\footnote{Geometrically, see Lemma 1.7 in
	\newcite{delpeuchNormalizationPlanarString2022}.}, we can see that their model
of string diagrams is just a rewriting of our model for vertical composition,
and allows for other reductions in the parser to reduce non-determinism.
...
diagrams to the ones of Section \ref{sec:nondet}.
What this means is that we could use the syntactic version of the diagrams to
model our parsing, according to the rules in Figure \ref{fig:combination-cfg},
and then combine the diagrams as in Equation \ref{eq:CSD}, as shown in Figure
\ref{fig:syntax-diagram}.
The obvious reaction\footnote{That I'm sure everyone have had seeing this.} is:
"\emph{Why is the type in the end not $\t$?}".
This is a perfectly valid point, and this is simply a notation abuse, from the
parsing going bottom up instead of top down.
In this figure we exactly see the sequence of reductions play out on the types
of the words.

\begin{figure}
	\centering
	\inputtikz{syntax-diagram}
	\caption{Representation of the Connection between Syntactic Diagrams and
		Effectful Reduction Diagrams}
	\label{fig:syntax-diagram}
\end{figure}
