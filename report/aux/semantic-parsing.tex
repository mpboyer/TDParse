...
The CFGs provided  in Figures \ref{fig:english-cfg} and \ref{fig:combination-cfg}
...
Now, note that this means all the improvements discussed in Section \ref{subsec:rewrite}
can still be applied here, just a bit differently, as this amounts to reducing
ambiguity in the grammar.

\begin{figure}
	\centering
	% TODO: expand the grammar below so that the number of rules is actually 
	% finite and so functorial application can only be done by blocks.
	% F1F2l · G1G2r cannot be F1G1F2G2 but only F1F2G1G2 or G1G2F1F2.
...
\begin{thm}
	...
	Generalized semantic parsing of a sentence is polynomial in the length of the
	sentence and the size of the type system and syntax system.
\end{thm}
\begin{proof}
...
\end{thm}
\begin{proof}
	We have proved in Theorem \ref{thm:ptime-parse} that we can retrieve all the
	semantic parse trees from a	sentence in polynomial time in the input.
...
For example, $S_{CFG}\left(G_{s}, n\right) = \abs{G_{s}}n^{3}$.
We cannot achieve a non-ambiguous syntactic structure for the English language,
so the number of parse trees will be exponential in the size of the sentence.
