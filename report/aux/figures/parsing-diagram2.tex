...
\rotatebox{90}{%
	\begin{tikzpicture}
		\pic[rotate=-90, transform shape] (nbox) at (0, 0) {cat};
		\pic[rotate=-90, transform shape] (na) at ($(nbox-north) + (-2, 4)$) {a};
		\node[rectangle, draw=black] (db) at ($(nbox-north) + (1, 0)$) {$\rotatebox{-90}{\f{D}(\e \to \t)}$};
		\node[rectangle, draw=black] (da) at ($(na-north) + (1, 0)$) {$\rotatebox{-90}{(\e \to \t) \to \f{D}\e}$};
		\node[rectangle, draw=black] (fmap) at ($(nbox-north) + (3, 1)$) {\rotatebox{-90}{$\combML_{\f{D}}$}};
		\path (fmap)
