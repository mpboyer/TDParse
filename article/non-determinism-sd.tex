...
the uneffectful denotation in $\mC$ while the \emph{functorial} string for
$\f{M}$ introduces the effect for optionality and possible failure of the
computation.
...
This gives a naïve algorithm in polynomial time to check if a string diagram is
valid or not.
...
data structure supports the linear and polynomial time algorithms proposed with
